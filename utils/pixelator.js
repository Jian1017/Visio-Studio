/**
 * Visio Studio 拼豆工坊 - 像素网格处理核心算法
 */

const { ARTKAL_C_COLORS, matchBeadColor, hexToRgb, getPerceptualDistance } = require('./colors.js');

/**
 * 核心：像素化并映射到拼豆色卡
 * @param {Object} canvas - 微信小程序 Canvas 2D 实例
 * @param {Object} ctx - Canvas 2D 渲染上下文
 ...
 * @param {Number} contrast - 对比度增强
 * @param {String} colorMode - 颜色模式 ('color' 或 'bw')
...
function processPixelData(canvas, ctx, sourceWidth, sourceHeight, targetSize, maxColors = 0, contrast = 0, colorMode = 'color') {
  // 1. 在临时小 Canvas 上获得像素（利用 canvas 缩放实现最近邻下采样）
  
  // 禁用抗锯齿以获得干净的像素效果 (ImageSmoothingEnabled = false)
  ctx.imageSmoothingEnabled = false;
  ctx.mozImageSmoothingEnabled = false;
  ctx.webkitImageSmoothingEnabled = false;
  ctx.msImageSmoothingEnabled = false;
  
  // 将源画布/图像绘制到目标尺寸的临时区域上
  // 这里假设传入的 ctx 已经绑定了一个尺寸为 targetSize x targetSize 的小 Canvas
  // 读出该 Canvas 的 ImageData
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      // 对比度增强算法 (Push pixels to extreme states to avoid anti-aliasing gray blur on logos)
      if (contrast > 0) {
      ...
      if (colorMode === 'bw') {
        ...
        // 如果亮度大于 127，映射为 H2 白色，否则为 H7 黑色
        const beadCode = (gray > 127) ? 'H2' : 'H7';
  ...
  // 如果设定了最大颜色限制，并且实际检测到的颜色数超出了限制
  const detectedCodes = Object.keys(beadHistogram);
  if (maxColors > 0 && detectedCodes.length > maxColors) {
  ...
  const beadCounts = Object.values(beadHistogram)
    .sort((a, b) => b.count - a.count)
    .map((item, index) => {
      // 直接使用 Artkal 色号作为代号，方便对照实物豆子包装标签
      let symbol = item.bead.code;
      return {
        ...item.bead,
        count: item.count,
        symbol: symbol
